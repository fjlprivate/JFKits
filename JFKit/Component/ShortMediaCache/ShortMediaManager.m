...
- (void)cleanCache {
    [[ShortMediaDownloader shareDownloader] cancelAllDownloads];
...
- (void)startPreloading {
    ...
    if(canPreload) {
        ...
        [[ShortMediaDownloader shareDownloader] preloadMediaWithUrl:url options:kNilOptions progress:nil completion:^(NSError *error) {
            ...
            if(!error) {
                NSLog(@"End preloading, NON error:::[%@]", url);
                Lock();
                [self.waitingPreloadingUrls removeObject:url];
                UnLock();
                [self startPreloading];
            } else {
                NSLog(@"End preloading, YES error:::[%@], error[%@]", url , error);
            }
        }];
    }
    else {
