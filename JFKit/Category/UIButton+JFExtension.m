...
static char* JFButtonExtensionAlignmentKey = "JFButtonExtensionAlignmentKey";
static char* JFButtonExtensionTypeKey = "JFButtonExtensionTypeKey";
static char* JFButtonExtensionPaddingKey = "JFButtonExtensionPaddingKey";

@interface UIButton ()
@property (nonatomic, assign) JFButtonAlignment jf_btn_alignment;
@property (nonatomic, assign) JFButtonType jf_btn_type;
@property (nonatomic, assign) CGFloat jf_btn_padding;
@end
                      ...
                      padding:(CGFloat)padding
{
    self = [super init];
    if (self) {
        objc_setAssociatedObject(self, JFButtonExtensionAlignmentKey, @(alignment), OBJC_ASSOCIATION_RETAIN);
        objc_setAssociatedObject(self, JFButtonExtensionTypeKey, @(type), OBJC_ASSOCIATION_RETAIN);
        objc_setAssociatedObject(self, JFButtonExtensionPaddingKey, @(padding), OBJC_ASSOCIATION_RETAIN);
...
- (void) updateContentsLayout {
    ...
    JFButtonAlignment alignment = [objc_getAssociatedObject(self, JFButtonExtensionAlignmentKey) integerValue];
    JFButtonType type = [objc_getAssociatedObject(self, JFButtonExtensionTypeKey) integerValue];
    CGFloat padding = [objc_getAssociatedObject(self, JFButtonExtensionPaddingKey) doubleValue];
    /* origin
     * image
     L: x
     R: title + x
    ...
    // 中心 + 上图下文
         ...
         * image L: x + title/2
         * image R: x + title/2
         * image T: y2 = (h - hi - ht - padding)/2 //y - image/2 - padding/2
         * image B: y2 + ht + padding
         * title L: x + image/2
         * title R: x + image/2
         * title T: y2 + hi + padding //z + title/2 + padding/2
         * title B: y2 //z - title/2 - padding/2
         */
